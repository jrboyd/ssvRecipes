...
star_align_fastq_PE = function(r1_fastq_paths,
                               ...
                               hold_jids = NA,
                               out_path = getwd(),
                               out_dirs = paste0(sub("_P1.fastq", "", basename(r1_fastq_paths)), ".STAR."),
                               do_submit = TRUE
){
    bfc = BiocFileCache::BiocFileCache(cache_path)
    if(is.list(index_path)) index_path = index_path[[1]]
    ...
    stopifnot(!any(duplicated(out_dirs)))
    stopifnot(length(hold_jids) == 1 | length(hold_jids) == length(r1_fastq_paths))
    if(length(hold_jids) == 1){
        hold_jids = rep(hold_jids, length(r1_fastq_paths))
    }
    cmd_align = "STAR_VAR \
    --runThreadN 8 \
    --genomeDir INDEX_VAR \
    ...
    for(i in seq_along(r1_fastq_paths)){
        hjid = hold_jids[i]
        cmd_this = cmd_align
        cmd_this = sub("FASTQ_VAR", paste(r1_fastq_paths[i], r2_fastq_paths[i]), cmd_this)
        cmd_this = sub("THREADS_VAR", p, cmd_this)
        cmd_this = sub("OUT_VAR", out_dirs[i], cmd_this)
